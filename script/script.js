...
They are based on your Singularity level.`
  ...
  get("totalOmegaChalComp").textContent=`Your ${getSumOC().toFixed(2)} Omega Challenge Completions are multiplying booster gain from milestones and OP gain by ${Math.max(getSumOC() + 1,1).toFixed(2)}`
...
function changeOrdLengthMoreMore() {
  let newms = prompt(
    "Please type in the new max length. Type in 0 for no maximum"
  );
  if (!isNaN(Number(newms))&&Number(newms)>=0) {
    game.maxOrdLength.moreMore = Math.round(Number(newms));
  }
}
...
function displayOrd(
  ...
  if (ord == Infinity) {
    ...
    let output = (colour == 1
      ...
      "x",
      trim == game.maxOrdLength.moreMore - 1
        ? colour == 1
          ? color("...", ["..."], HSL(tempvar * 8))
          : "..."
...
function resetConf() {
  ...
  if (code !== null && code.toLowerCase() == "yes") {
    reset();
...
function maximizeSing() {
  while (game.darkManifolds - getDMSacrifice() >= 1e6 * (game.sfBought.includes(23) ? 4: 5) ** game.sing.dm) game.sing.dm++;
