...
function calcCardExponent(s) {
  return 0.5 + (5 - Math.min(Math.log10(Math.max(1000, s)), 5)) ** 1.5;
}

function getManifoldEffect() {
  return (
    (Math.max(1, game.manifolds + 1 - game.sing.m) ** 0.5 *
    (game.iups[4 - 1] === 1 ? 3: 1) *
    (game.iups[4] === 1 ? 1.26: 1)) **
    (game.sfBought.includes(62) ? 4: 1)
  );
}
...
function getDarkManifoldEffect() {
  return (
    Math.max(1, game.darkManifolds + 1 - getDMSacrifice()) ** 0.1 *
...
function calcIncrementyMult(i = game.incrementy) {
  let k = EN(i)
    .add(10)
    .log10()
    .times(EN(EN(1.2).times(game.sfBought.includes(102) ? calcSF102Effect(): 1)).pow(game.iups[2]))
    .times(1.2)
...
// hi
function getSumOC() {
  return getOCComp(1) + getOCComp(2) + getOCComp(3) + getOCComp(4) + getOCComp(5) + getOCComp(6);
}
